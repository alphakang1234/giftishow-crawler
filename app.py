...
def fetch_all_giftishow_products_safely():
    ...
    while True:
        # 🔥 마케터님의 핵심 단서: start와 size를 반드시 "문자열" 형태로 전송!
        payload = {
            "start": str(start_index),
            "size": str(page_size),
            "lineUp": "popular",
            "categoryList": ["1"],      # 기본적으로 커피/음료 카테고리 타겟팅
            "categoryName": ["커피/음료"]
        }
        ...
        try:
            ...
            # 마케터님이 검증하신 성공 조건 '0000' 적용
            if json_data.get('code') == '0000':
                # 진짜 데이터 주머니 이름인 'list'에서 상품 솎아내기
                ...
                # ⏱️ [마케터님 핵심 요구사항] 다음 페이지 재호출 전 정확히 3초(+알파) 랜덤 텀 두기!
                # 기계적인 차단을 완전히 우회하기 위해 3.0초에서 4.2초 사이로 유연하게 쉽니다.
                sleep_time = 3.0 + random.uniform(0, 1.2)
                status_box.info(f"🔄 현재 {current_count}개 상품 수집 완료... IP 차단 방지를 위해 {sleep_time:.1f}초간 대기합니다. ⏳")
                
                # 다음 200개를 가져오기 위해 인덱스 점프 (1 -> 201 -> 401...)
